...
class SessionStore:
    ...
    def append_message(
        ...
        content: str,
        *,
        transcript_blocks: list[dict[str, Any]] | None = None,
        ...
        if transcript_blocks:
            entry["transcript_blocks"] = transcript_blocks
    ...
    def load_cumulative_transcript_blocks(self) -> tuple[PresentationBlock, ...]:
        """Rebuild append-only transcript history from stored assistant turns."""
        cumulative: tuple[PresentationBlock, ...] = ()
        for message in self.load_conversation():
            if message.get("role") != "assistant":
                continue
            raw_blocks = message.get("transcript_blocks") or []
            if not isinstance(raw_blocks, list):
                continue
            turn_blocks = tuple(
                presentation_block_from_dict(item)
                for item in raw_blocks
                if isinstance(item, dict) and item.get("block_id")
            )
            cumulative = append_transcript_blocks(cumulative, turn_blocks)
        return cumulative
