...
`storage/` implements **desktop project persistence**: SQLite schema/connections, CRUD for projects/tasks/chat/artifacts, one-time migration from legacy filesystem sessions, and a **session-compatible store** (`ProjectSessionStore`) that mirrors `storage/session_store.SessionStore` while writing to both SQLite and the existing `sessions/` directory layout.
...
| `get_database_for_config()` | Callable | Used by `api/desktop_service.py` |
...
| --- | --- | --- |
| `storage/session_store.py` | `project_session_store.py` | `_task_from_dict`, `_task_to_dict`, `_input_from_dict`, `_input_to_dict` — shared serialization |
...
Desktop API and `storage/session_store.py` cover session persistence; CLI entry point removed.
...
| --- | --- | --- |
| `SessionStore` | `storage/session_store.py` | Filesystem (`sessions/<id>/tasks.json`, `conversation.json`) |
| `ProjectSessionStore` | `storage/project_session_store.py` | SQLite **plus** same filesystem paths |

`ProjectSessionStore` **depends on** `storage/session_store` serialization helpers — not duplicated.
...
| **Imported by** | `project_session_store.py`, `migrate_legacy_sessions.py`, `api/desktop_service.py`, tests |
...
| **Imported by** | `project_session_store.py`, `api/desktop_service.py`, tests |
...
| **Imported by** | `api/desktop_service.py`, `api/report_service.py`, tests |
| **Imports** | `storage/session_store`, `engine/state/state_manager`, `models/task`, `storage/*` |
...
storage/session_store._task_from_dict  →  models/task.Task
    ↓
...
ProjectSessionStore.save_state_manager()
    ↓
storage/session_store._task_to_dict  →  models/task.Task serialized
    ↓
...
### Trace 7 — CLI path (does **not** use `storage/`)
...
| --- | --- | --- |
| Session/task persistence | `storage/session_store.SessionStore` | `storage/project_session_store.ProjectSessionStore` |
| Task (de)serialization | `storage/session_store._task_from_dict/_task_to_dict` | Same functions reused by B |
| Project listing | `SessionStore.list_sessions()` (directory names) | `ProjectRepository.list_projects()` / `list_project_summaries()` |
| JSON default handler | `storage/session_store._json_default` | `project_repository._json_default` / `project_session_store._json_default` (three near-identical copies) |
