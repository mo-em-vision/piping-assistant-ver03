...
def test_build_plan_minimal_nodes_while_expansion_gate_closed() -> None:
    reader = _reader()
    ...
    assert "WF-PIPE-WALL-THICKNESS" in plan.nodes
    assert "304.1.2-a" not in plan.nodes
    assert "asme-b313-304-1-1-eq-2" not in plan.nodes
...
def test_discover_roots_wall_thickness() -> None:
    ...
    assert candidates
    assert all(candidate.root_id == "pipe_wall_thickness_design" for candidate in candidates)
    assert candidates[0].confidence >= 0.85


def test_required_user_inputs_empty_while_gate_closed() -> None:
    reader = _reader()
