...
def test_workflow_node_transcript_blocks_emit_workflow_intro(
    ...
    assert intro_block.source == "workflow_node"
    assert intro_block.payload.get("display_role") == DisplayRole.workflow_intro.value
    assert intro_block.payload.get("title") == "Pipe Wall Thickness Design"
    assert intro_block.text.startswith("Pipe Wall Thickness Design")
    assert "graph expansion" in (intro_block.text or "").lower()


def test_create_task_transcript_includes_workflow_intro(
    tmp_path: Path,
    project_root: Path,
) -> None:
...
def test_awaiting_input_does_not_emit_input_waiting_scroll_block(
    tmp_path: Path,
    project_root: Path,
    standards_reader,
) -> None:
...
def test_center_panel_scroll_orders_workflow_intro_first(
    standards_reader,
) -> None:
    from engine.state.state_manager import TaskStateManager
    ...
    assert roles[0] == DisplayRole.workflow_intro.value
