...
CALCULATION_NODES = ("B313-304.1.1",)
...
@pytest.mark.parametrize("node_id", ("B313-material-stress", "B313-304.1.1"))
def test_calculation_nodes_have_formula_or_lookup(node_id: str, standards_reader: StandardsReader) -> None:
    ...
    if node_type == "calculation":
        assert record.metadata.get("formulas"), f"{node_id} missing formulas"
    if node_type == "lookup":
