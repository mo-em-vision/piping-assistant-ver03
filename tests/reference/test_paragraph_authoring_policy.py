...
@pytest.mark.parametrize(
    "key",
    ["applicability", "trace", "report", "ai_hints"],
)
def test_forbidden_paragraph_frontmatter_fails(key: str) -> None:
    findings = check_paragraph_frontmatter_placement(
        {"type": "paragraph", key: {"applies_when": []} if key == "applicability" else "x"},
        node_id="304.1.2-a",
    )
    assert any(level == "FAIL" for level, _ in findings)


def test_sidecar_only_assumptions_warns_in_phase_1() -> None:
    assert SIDECAR_ONLY_ENFORCEMENT == "warn"
    assert sidecar_only_severity() == "WARN"
    findings = check_paragraph_frontmatter_placement(
        {"type": "paragraph", "assumptions": [{"id": "a"}]},
        node_id="304.1.1-a",
    )
    assert findings == [
        ("WARN", "assumptions belongs in execution sidecar; migration required")
    ]
...
def test_validator_forbidden_union_matches_policy() -> None:
    ...
    assert any("applicability" in m for m in messages)
    assert any("runtime_value" in m for m in messages)
    assert not any("assumptions" in m for m in messages)
...
def test_forbidden_sets_do_not_overlap_runtime_and_sidecar_only() -> None:
    ...
    overlap = FORBIDDEN_PARAGRAPH_FRONTMATTER & SIDECAR_ONLY_KEYS
    assert overlap == {"applicability"}
