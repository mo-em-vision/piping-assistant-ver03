...
def test_build_pack_dedupes_by_nested_path(tmp_path: Path, project_root: Path) -> None:
    ...
    nested = pack / "nodes" / "304" / "304.1.1"
    assert nested.is_dir()
    flat.mkdir(parents=True, exist_ok=True)
    (flat / "node.md").write_text(
        "---\nid: B313-304.1.1\ntype: calculation\n---\nFlat duplicate\n",
        encoding="utf-8",
    )
    ...
    assert "304/304.1.1" in node["source_rel_path"]
