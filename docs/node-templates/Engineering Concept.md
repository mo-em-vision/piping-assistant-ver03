...
Do **not** use `category` or `categorical` as `concept_class` values. They are too broad for semantic grouping; use [`parameter_class`](Parameter%20Node.md) on Parameter nodes and categorical Fact value shapes instead (see below).

---

# Selection vs categorical (where each belongs)

| Layer | Field | Role |
|-------|-------|------|
| Concept | `concept_class: selection` | Semantic idea of classifying something for standards resolution (joint category, service category). |
| Parameter | `parameter_class: selection` | Discrete user/workflow choice; drives confirmation prompts and table lookups. See [Parameter Node](Parameter%20Node.md). |
| Parameter | `parameter_class: categorical` | Enumerated designation or label-driven behavior without a dedicated selection concept. |
| Fact | categorical value `{label, normalized_key}` | Runtime stored value (material spec, joint category label). See [Fact Node](Fact%20Node.md). |
| Dimension | `dimension_kind: categorical` | Unit compatibility for designation-like parameters. See [Dimension Node](Dimension%20Node.md). |

Concepts define **what** the engineering idea is. Parameters define **which role** and **how behavior is controlled** at runtime.
...
# Semantic concept example (material)

Material is a **semantic** concept (`concept_class: material`), not a categorical one. Runtime behavior and designation resolution belong on [`PARAM-material-specification`](../../knowledge/global/parameters/nodes/PARAM-material-specification.yaml) (`parameter_class: material_designation`).
...
Lookup behavior and dropdown options are defined on [`PARAM-joint-category`](../../knowledge/global/parameters/nodes/PARAM-joint-category.yaml) (`parameter_class: selection`), not on the Concept.
...
Defines contextual engineering role and behavior class (`parameter_class`).
...
Which joint category drives the E lookup?
```

See [Parameter Node](Parameter%20Node.md) for `selection` and `categorical` behavior classes.
...
6. A non-physical concept class (`material`, `selection`, `coefficient`, …) references a physical `DIM-*` without justification. Categorical **behavior** is validated on Parameter and Fact nodes, not Concept.
    
7. `concept_class` is `category` or `categorical`.
    
8. It stores runtime values.
    
9. It duplicates another Concept with the same semantic meaning.
    
10. It defines execution logic directly.
    
11. It replaces a Parameter where contextual role is required.
...
CONCEPT-joint-category
  ├── PARAM-joint-category
  └── B313-table-A-3 (used_by)
```