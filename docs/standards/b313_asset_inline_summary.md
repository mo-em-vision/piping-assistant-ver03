...
| Node | Before | After |
|------|--------|-------|
| `304.1.2-a` | `equations/`, `conditions/`, `notes/`, `references/` | `node.yaml` only |
| `B313-304.1.1` | `equations/` | `node.yaml` only |
| `302.3.5-e` | `node.md` + `equations/` (md + py) | `node.yaml` + `eq_1a_*.py`, `eq_1b_*.py`, `eq_1c_*.py` at root |
| `304.3.3-a` | `node.md` + `equations/` | `node.yaml` only |
...
- `304.1.2-a`: `wall_thickness.md`, `mawp_pressure.md`, `thick_wall_y.md`, `thin_wall_check.md`, corrosion/mill notes, `material_stress.md` reference → embedded `source:` on metadata entries
- `B313-304.1.1`: `eq_2_minimum_required_thickness.md` → subsection `equations[].source`
- `302.3.5-e`: three equation `.md` files → `equations[].source`; `.py` modules moved to node root
- `304.3.3-a`: four reinforcement equation `.md` files → `equations[].source`
...
| Old path | Updated |
|----------|---------|
| `../304.1.2/equations/mawp_pressure.md` | `nodes/304.1.2-a/equations/mawp_pressure.md` (alias → embedded source) |
...
- `304.1.2-a`: `interactions`, `inputs` (from legacy `304/304.1/304.1.2/node.md`)
