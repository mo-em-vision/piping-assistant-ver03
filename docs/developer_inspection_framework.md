...
| Frontend gate | `env.devMode` (`VITE_DEV_MODE=true` or Vite `DEV`) — inspector UI not loaded in production |
| Tree-shaking | `DeveloperInspector` is lazy-imported in `WorkspaceLayout.tsx`; omitted from release bundle |
| Runtime impact | Instrumentation is read-only; optional **breakpoints** only pause the executor loop between nodes |
| Electron dev | `DEV_INSPECTION_ENABLED=1` is set automatically when `DEV_STUDIO_ENABLED=1` (unpackaged builds) |
...
| `VITE_DEV_MODE` | Frontend | Enables dev badge, inspector toggle, provenance hovers |
| `DEV_STUDIO_ENABLED` | Backend | Enables Node Dev Studio (separate concern; dev Electron sets both) |
...
```
desktopApp/src/components/dev/inspector/
...
# Frontend
cd desktopApp && npm run test:run -- tests/components/dev/ExecutionTracePanel.test.tsx
```
