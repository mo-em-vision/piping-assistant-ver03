...
Agents are used only where interpretation is required.  
  
  
The agent acts as an engineering navigator.  
  
  
The agent helps determine:  
...
After graph execution begins, deterministic systems control the process.  
...
```
ASME B31.3 integrity root node  
```

  
Requests missing inputs  
  
The graph engine then executes.  
...
Agents determine navigation only.
...
A session may contain multiple engineering tasks.
...
If a new task changes an existing input:
...
```
"The previous calculation may no longer be valid with the updated parameters. A complete recalculation is required."
```
...
The report generator supports multiple outputs:
...
The underlying report data model remains identical.


Only presentation format changes.
